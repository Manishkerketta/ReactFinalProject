...
import { Bell, ChevronDown, User, KeyRound, LogOut, Key, Eye, EyeOff, BellRing, CheckCircle2, AlertCircle, Loader2 } from 'lucide-react';
import { sendChangePasswordOtp, performLogout } from '../../utils/authService';
...
const Header = ({ toggleSidebar }) => {
  ...
  const [respModal, setRespModal] = useState({ show: false, success: false, message: '', type: 'general' });
  ...
  const handleLogoutAction = async () => {
    setLoading(true);
    try {
      const resp = await performLogout();
      const isSuccess = resp.data.status === "SUCCESS";

      setRespModal({
        show: true,
        success: isSuccess,
        message: resp.data.statusDesc || "You have logged out successfully",
        type: 'logout'
      });
  ...
  const handleCloseRespModal = () => {
    const isLogout = respModal.type === 'logout';
    setRespModal({ ...respModal, show: false });
  ...
  const handlePasswordSubmit = async (e) => {
    ...
    if (passData.old === passData.new) {
      return setRespModal({ show: true, success: false, message: "New password cannot be same as current.", type: 'password' });
    }
    if (passData.new !== passData.confirm) {
      return setRespModal({ show: true, success: false, message: "Passwords do not match.", type: 'password' });
    }
    ...
    try {
      ...
      if (isSuccess) setActiveModal(null);
    } catch (err) {
      setRespModal({
        show: true,
        success: false,
  ...
  return (
    <>
      <header className="h-[70px] bg-[#8B0000] text-white flex items-center justify-between px-6 relative z-30 shadow-md">
        ...
        <div className="flex items-center gap-4">
          <button onClick={() => setActiveModal('notifications')} className="p-2 hover:bg-white/10 rounded-full relative group">
            <Bell size={20} />
          </button>
      ...
      <AnimatedModal isOpen={activeModal === 'notifications'}>
        <div className="p-8 text-center">
          ...
          <p className="text-gray-500 text-sm mb-8 leading-relaxed">Feature will be available in the upcoming system enhancement.</p>
          <button 
            onClick={() => setActiveModal(null)} 
            className="w-full py-3 bg-[#8B0000] text-white font-bold rounded uppercase tracking-widest text-xs hover:bg-[#700000] transition-all shadow-md"
          >
            Got it
          </button>
        </div>
      ...
      {/* --- RESPONSE STATUS MODAL (For Password & Logout Responses) --- */}
      {respModal.show && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-[400px] p-10 text-center animate-modalIn">
            ...
            <h3 className="text-xl font-bold text-gray-800 mb-2">{respModal.success ? "Success" : "Error"}</h3>
            <p className="text-gray-500 text-sm mb-8 leading-relaxed">{respModal.message}</p>
            <button onClick={handleCloseRespModal} className="w-full py-3 bg-[#8B0000] text-white font-bold rounded uppercase tracking-widest text-xs hover:bg-[#700000] transition-all">
              {respModal.type === 'logout' ? "Redirect to Login" : "Close"}
            </button>
          </div>
        </div>
      )}
      ...
      <AnimatedModal isOpen={activeModal === 'logout'}>
        <div className="p-10 text-center">
          ...
          <p className="text-gray-500 text-sm mb-10 leading-relaxed">Your session will be securely ended. Do you wish to continue?</p>
          <div className="flex gap-4">
            <button 
              disabled={loading}
              onClick={handleLogoutAction} 
              className="flex-1 py-3 bg-[#8B0000] text-white font-bold rounded uppercase hover:bg-[#700000] transition-all flex items-center justify-center"
            >
              {loading ? <Loader2 className="animate-spin" size={18} /> : "Yes, Logout"}
            </button>
      ...
      <AnimatedModal isOpen={activeModal === 'profile'}>
        ...
        <div className="px-8 pb-8"><button onClick={() => setActiveModal(null)} className="w-full py-3 bg-[#8B0000] text-white font-bold rounded hover:bg-[#700000] transition-colors uppercase tracking-widest text-sm shadow-md">Close</button></div>
      </AnimatedModal>
      ...
      {/* <AnimatedModal isOpen={activeModal === 'changePassword'}>
        <div className="p-8">
          <div className="text-center mb-8">
            <h2 className="text-2xl font-bold text-gray-800">Update Credentials</h2>
            <p className="text-sm text-gray-500 mt-1">Please enter your current and new password</p>
          </div>
          <form className="space-y-4" onSubmit={handlePasswordSubmit}>
            {['old', 'new', 'confirm'].map((id) => (
              <div key={id} className="relative">
                <input required type={showPassword[id] ? "text" : "password"} placeholder={id === 'old' ? 'Current Password*' : id === 'new' ? 'New Password*' : 'Confirm New Password*'} className="w-full px-4 py-3.5 border border-gray-200 rounded-lg text-sm outline-none focus:border-red-800 transition-all" value={passData[id]} onChange={(e) => setPassData({ ...passData, [id]: e.target.value })} />
                <button type="button" onClick={() => setShowPassword(prev => ({ ...prev, [id]: !prev[id] }))} className="absolute right-3 top-4 text-gray-400 hover:text-gray-600">
                  {showPassword[id] ? <Eye size={18} /> : <EyeOff size={18} />}
                </button>
              </div>
            ...
            <div className="flex gap-4 pt-6">
              <button type="button" onClick={() => setActiveModal(null)} className="flex-1 py-3 border-2 border-gray-200 text-gray-500 font-bold rounded-lg hover:bg-gray-50 transition-all">Cancel</button>
              <button disabled={loading} type="submit" className="flex-1 py-3 bg-[#8B0000] text-white font-bold rounded-lg shadow-lg hover:bg-[#700000] flex items-center justify-center transition-all">
                {loading ? <Loader2 className="animate-spin" size={20} /> : "Update Password"}
              </button>
            </div>
          </form>
